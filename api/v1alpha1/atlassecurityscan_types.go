...
// MinSeverity is the lowest level the scan reports. It is always set, so the
// severity the report is graded with is the one the CLI ran with.
func (s *AtlasSecurityScan) MinSeverity() SecurityLevel {
	if p := s.Spec.Policy; p != nil && p.MinSeverity != "" {
		return p.MinSeverity
	}
	return SecurityLevelNormal
}
...
func LevelIndex(l SecurityLevel) int {
	return slices.Index(SecurityLevels, l)
}
