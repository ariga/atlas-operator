...
func (r *AtlasSchemaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, err error) {
	...
	// This function is used to re-render the atlas.hcl file in the working directory.
	overrideAtlasHCL := func(fn func(*managedData)) error {
		fn(data)
	...
	case whoami != nil:
		err = overrideAtlasHCL(func(m *managedData) {
...
// When set to "unset," the policy is only enabled if the configuration isn't already defined.
func (d *managedData) enableDestructive(force bool) {
	destructive := &dbv1alpha1.Lint{
		Destructive: &dbv1alpha1.CheckConfig{
			Error: true,
		},
	}
	if d.Policy == nil {
		d.Policy = &dbv1alpha1.Policy{
			Lint: destructive,
		}
		return
	}
	...
	if d.Policy.Lint.Destructive == nil {
		d.Policy.Lint.Destructive = &dbv1alpha1.CheckConfig{
			Error: true,
		}
		return
	}
	// When "force" is true, we force the policy to be enabled.
	if force {
		d.Policy.Lint.Destructive.Error = true
	}
}
...
func (d *managedData) setLintReview(v dbv1alpha1.LintReview, force bool) {
	...
	if d.Policy.Lint == nil {
		d.Policy.Lint = &dbv1alpha1.Lint{
			Review: v,
		}
		return
	}
	if d.Policy.Lint.Review == "" {
