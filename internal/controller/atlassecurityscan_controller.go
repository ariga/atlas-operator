...
func (r *AtlasSecurityScanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, err error) {
	...
	// Inputs that no scan can fix stall the resource until the spec changes.
	...
	snap := r.snapshot(ctx, res, sched, now)
	...
	// A stalled resource makes one attempt per distinct set of inputs.
	if res.IsStalled(dbv1alpha1.ReasonBackoffLimitExceeded) && res.Status.LastScan != nil && snap.hash == res.Status.LastScan.InputsHash {
		return r.wake(res, sched, now, 0), nil
	}
	// Start: record the attempt, commit no watermark yet.
	if res.WasSuspended() {
	...
	if err != nil {
		return r.fail(ctx, res, sched, dbv1alpha1.ReasonReadingInputs, err), nil
	}
	...
	if err != nil {
		return r.fail(ctx, res, sched, dbv1alpha1.ReasonCLIError, err), nil
	}
	if data.Cloud != nil && data.Cloud.Token != "" {
		if err := cli.Login(ctx, &atlasexec.LoginParams{Token: data.Cloud.Token, GrantOnly: true}); err != nil {
			return r.fail(ctx, res, sched, dbv1alpha1.ReasonLoginFailed, err), nil
		}
	}
	...
	switch {
	case scan == nil:
		return r.fail(ctx, res, sched, dbv1alpha1.ReasonCLIError, serr), nil
	case len(scan.Targets) == 0:
		return r.fail(ctx, res, sched, dbv1alpha1.ReasonScanFailed, errors.New("the report names no target")), nil
	case len(scan.Targets) > 1:
		...
		res.SetStalled(dbv1alpha1.ReasonInvalidTarget, msg)
		r.recorder.Event(res, corev1.EventTypeWarning, dbv1alpha1.ReasonInvalidTarget, msg)
		return r.wake(res, sched, r.now(), 0), nil
	case len(scan.Failures()) > 0 || scan.Targets[0].Error != "":
		return r.fail(ctx, res, sched, dbv1alpha1.ReasonScanFailed, errors.New(scan.Targets[0].Error)), nil
	case serr != nil:
		// e.g. a custom config with security { fail_on }: the target was scanned.
	...
	var (
		...
		findings, summary     = grade(target, policy, snap.waivers)
		verdict, reason, vmsg = verdictOf(findings, policy, res.Name)
		completion            = r.now()
		report                = r.buildReport(res, target, findings, summary, snap, now, completion, trigger)
	)
	if err := r.storeReport(ctx, res, report); err != nil {
		return r.fail(ctx, res, sched, dbv1alpha1.ReasonStoringReport, err), nil
	}
	...
	case snap.slot != nil:
		st.LastScheduleTime = &metav1.Time{Time: *snap.slot}
	}
	...
	if trigger != dbv1alpha1.TriggerSpec && prevSlot != nil && snap.slot != nil {
		if n := slotsBetween(sched, prevSlot.Time, *snap.slot); n > 0 {
			r.recorder.Eventf(res, corev1.EventTypeNormal, dbv1alpha1.EventMissedSchedule,
				"caught up %d missed slots; latest covered %s", n, snap.slot.UTC().Format(time.RFC3339))
		}
	}
...
// revisionChanged passes only the updates where the applied revision changed,
// the same comparison the due-check makes, so an event is always a pending trigger.
...
func (r *AtlasSecurityScanReconciler) snapshot(ctx context.Context, res *dbv1alpha1.AtlasSecurityScan, sched *scanSchedule, now time.Time) *scanSnapshot {
	snap := &scanSnapshot{
	...
	if p := res.Spec.Policy; p != nil {
		for _, w := range p.Ignore {
			if w.ExpirationTime == nil || w.ExpirationTime.After(now) {
				snap.waivers = append(snap.waivers, w.ID)
			}
		}
		slices.Sort(snap.waivers)
	}
	h := sha256.New()
...
// expiry and the retry delay, floored at one second. It is computed from now on
// every path, never from nextScheduleTime, which may sit in the past while a
// slot is uncommitted. Without any of the three, the resource is event-driven.
...
// fail records a failed attempt. It is retried with the existing linear backoff
// until the limit, after which the resource stalls and makes one attempt per new
// set of inputs. The cause goes to the log only: CLI and driver errors carry the
// resolved address of a Secret-held host, which status and Events must not.
func (r *AtlasSecurityScanReconciler) fail(ctx context.Context, res *dbv1alpha1.AtlasSecurityScan, sched *scanSchedule, reason string, cause error) ctrl.Result {
	now := r.now()
	res.Status.Failed++
	msg := failureText(reason, res.Status.Failed, res.Spec.BackoffLimit)
	closeAttempt(res, now, dbv1alpha1.ScanFailed, msg)
	log.FromContext(ctx).Error(cause, "security scan failed", "reason", reason, "attempt", res.Status.Failed)
	switch {
	// Conditions do not move while stalled, so a database down for a week reads
	// as Stalled once rather than flapping at every slot.
	case res.IsStalled(""):
		return r.wake(res, sched, now, 0)
	...
	return r.wake(res, sched, now, backoffDelayAt(res.Status.Failed))
}
...
func (r *AtlasSecurityScanReconciler) extractData(ctx context.Context, res *dbv1alpha1.AtlasSecurityScan) (_ *scanData, err error) {
	var (
		s    = res.Spec
		data = &scanData{EnvName: defaultEnvName}
	)
	if p := s.Policy; p != nil {
		data.MinSeverity = string(p.MinSeverity)
	}
	if s.EnvName != "" {
...
// grade attaches waivers to the findings and counts the rest per level.
func grade(t *atlasexec.SecurityScanTarget, policy *dbv1alpha1.ScanPolicy, waivers []string) ([]dbv1alpha1.ReportedVulnerability, dbv1alpha1.ScanSummary) {
	var (
		findings []dbv1alpha1.ReportedVulnerability
		counts   = make(map[dbv1alpha1.SecurityLevel]int32, len(dbv1alpha1.SecurityLevels))
		summary  = dbv1alpha1.ScanSummary{Driver: t.Driver, Extensions: int32(len(t.Extensions))}
...
func (r *AtlasSecurityScanReconciler) buildReport(
	...
	findings []dbv1alpha1.ReportedVulnerability, summary dbv1alpha1.ScanSummary,
	snap *scanSnapshot, start, completion time.Time, trigger dbv1alpha1.ScanTrigger,
	...
	policy := dbv1alpha1.GradedPolicy{MinSeverity: dbv1alpha1.SecurityLevelNormal}
	if p := res.Spec.Policy; p != nil {
		if p.MinSeverity != "" {
			policy.MinSeverity = p.MinSeverity
		}
		policy.FailOn = p.FailOn
	}
	exts := slices.Clone(t.Extensions)
	slices.Sort(exts)
	return &dbv1alpha1.AtlasSecurityReport{
		...
		Report: dbv1alpha1.SecurityReport{
			...
			Policy:          policy,
			Summary:         summary,
			Extensions:      slices.Compact(exts),
